...
def write_evaluate_script(dataset_reader, output_model, dataset):
    evaluate_filename = 'scripts/evaluate.sh'

    with open(evaluate_filename, 'w') as f:
        f.write('#!/bin/bash\n')
        f.write('allennlp evaluate ' + output_model + ' ' + dataset + ' --include-package drop_library --cuda-device 0 -o \'' + dataset_reader + '\'\n')
...
def main():
    ...
    # Run the evaluation
    subprocess.call(['./scripts/evaluate.sh'])
