...
@Model.register("naqanetic_bilstm")
class NumericallyAugmentedQaNetImprovedCountingBiLSTM(Model):
   """
   ...
   def forward(self,  # type: ignore
       ...
       if "counting" in self.answering_abilities:
           ...
           ### CHANGE: using question vectors instead of passage vectors ###
           ...
           # Shape: (batch_size, # of words, embedding_dim + question_vector_dim)
           encoded_words = torch.cat(
               [embedded_words, question_vector.unsqueeze(1).repeat(1, embedded_words.size(1), 1)], -1)
